...
UPGRADE UI
STM32Nucleo buffer
This is UI for project_23_03_27
...
class Ui_Form(object):
    ...
    def save(self):
        ...
        if (self.saveOption.currentText() == "TXT"):
            ...
            header = ["form", "input1", "input2", "input3", "input4", "input5", "input6", "input7", "input8", "input9", "input10"]
            ...
            df.to_csv("{}".format(fileName[0], index=False))


        elif (self.saveOption.currentText()=="XLSX"):
            header = ["input1", "input2", "input3", "input4", "input5", "input6", "input7", "input8", "input9", "input10"]
            ...
            fileName = QFileDialog.getSaveFileName(self.output_CSV, 'Select location to Log', "", '*.xlsx')

            if not fileName:
                return

            df.to_excel("{}".format(fileName[0], index=False))
    ...
    def read0(self):
        mytext_byte = [123, 82, 68, 48, 125, 13, 10]  # {RB1}\r\n
        ser.write(mytext_byte)
