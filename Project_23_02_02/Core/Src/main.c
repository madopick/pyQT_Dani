/* This Program is used for Accept input from user to control
 * LED lamp on STM32 Nucleo 446RE
 * Write and Read From UI Python on UI_23_02_02
*/
...
void HAL_UARTEx_RxEventCallback(UART_HandleTypeDef *huart, uint16_t Size)
{
  ...
  if (Size != old_pos)
  {
    ...
    if(cek(pBufferReadyForUser, "on")==0){
		ledDelay = 0;
	}
	else if(cek(pBufferReadyForUser, "fast")==0){
		ledDelay = 100;
	}
	else if(cek(pBufferReadyForUser, "slow")==0){
		ledDelay = 1000;
	}
...
int cek(char sumber[], char cari[]){
    char* posisi;

    posisi = strstr(sumber, cari);

    //Kondisi jika str tidak ditemukan
    if((int) sumber == (int) posisi - (int)sumber || (int) sumber == (-1)*((int) posisi - (int)sumber)){
        return -1;
    }
